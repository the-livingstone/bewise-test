...
class ApplicationRepo(BaseSqlaRepo[ApplicationORM]):
    ...
    async def get(self, id: str):
        result: ApplicationORM = await self.session.scalar(select(ApplicationORM).where(ApplicationORM.id==id))
        return result.from_orm() if result is not None else None

    async def get_many(self, username: str, page: int, page_size: int):
        results: list[ApplicationORM] = await self.session.scalars(self.query(username, page, page_size))
